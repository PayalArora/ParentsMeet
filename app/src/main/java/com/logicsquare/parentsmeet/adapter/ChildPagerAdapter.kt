...
class ChildPagerAdapter(fragment: Fragment, val kidsList: List<KidsItem?>?) : FragmentStateAdapter(fragment) {

    override fun getItemCount(): Int = kidsList?.size?:0

    override fun createFragment(position: Int): Fragment {
        // Return a NEW fragment instance in createFragment(int)
        val fragment = ChildPagerFragment(kidsList?.get(position))
